...
<body>
    ...
    <table role=""presentation"" cellspacing=""0"" cellpadding=""0"" border=""0"" width=""100%"" style=""background-color: #f9f9f9;"">
        ...
        body = body.Replace("{UserName}", user.FullName.Value);
        body = body.Replace(
            "{ResetPasswordUrl}",
            $"http://localhost:4200/reset-password/{user.ForgotPasswordId!.Value}");

        await mailService.SendAsync(to, subject, body);

        return "Mail gönderildi. Mail adresinizi kontrol ediniz.";
    }
}
